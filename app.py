...
def plot_probability_curve(total_score, prob_case):
    fig, ax = plt.subplots(figsize=(8, 4))   # 缩小显示
...
# 4. 合并图片函数（供下载）
# ============================================================
def combine_figures(fig_nomogram, fig_curve, dpi=600):
    """
    将两个 matplotlib 图垂直拼接为一张 PNG 图片，返回 BytesIO
    """
    buf1 = BytesIO()
...
# 下载按钮
buf = combine_figures(fig_nomogram, fig_curve, dpi=600)
st.download_button(
